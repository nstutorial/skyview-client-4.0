...
const examApi = {
    ...
    getExamConfig: async (className, section, academicYear) => {
        const response = await apiCall(`${API_BASE_URL}/exam/config/${encodeURIComponent(className)}/${encodeURIComponent(section)}/${encodeURIComponent(academicYear)}`);
        if (!response || !response.ok) throw new Error('Failed to fetch exam configuration');
        return response.json();
    },
    // Save exam configuration
    saveExamConfig: async (data) => {
        ...
        if (!response || !response.ok) throw new Error('Failed to save exam configuration');
        return response.json();
    }
};
...
const routineApi = {
    // Save routine
    saveRoutine: async (classname, day, periods) => {
        const response = await apiCall(`${API_BASE_URL}/routines/save-routine`, {
            method: 'POST',
            body: JSON.stringify({
                class: classname,
                day: day,
        ...
        if (!response || !response.ok) throw new Error('Failed to save routine');
        return response.json();
    },
    
    // Fetch routine by day
    getRoutineByDay: async (day) => {
        const response = await apiCall(`${API_BASE_URL}/routines/get-routine?day=${encodeURIComponent(day)}`);
        if (!response || !response.ok) throw new Error('Failed to fetch routine');
        return response.json();
    }
};
...
window.API = {
    ...
    marks: marksApi,
    exam: examApi,
    routine: routineApi // Added routine API
};
